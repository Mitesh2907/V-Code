...
const RoomsPage = () => {
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      try {
        const createdRes = await api.get("/rooms/created");
        const joinedRes = await api.get("/rooms/joined");
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="p-6 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            Create a new room and start collaborating
          </p>
        ...
        <div className="p-6 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
            Join an existing room using room number
          </p>
              ...
              {createdRooms.map(r => (
                <div key={r.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-900 rounded">
                  <div>
                    ...
                    <div className="text-xs text-gray-400">{r.room_number}</div>
                  </div>
                  <Button variant="ghost" size="sm" onClick={() => enterRoom(r.id)}>
                    Enter
                  </Button>
                </div>
              ))}
